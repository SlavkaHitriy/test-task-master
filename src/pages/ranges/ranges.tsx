...
const transform = (items: Item[]) => {
  const resRanges: Range[] = []
  const tempRanges: Range[] = []

  items.forEach(item => {
    const foundItem = tempRanges.find(tempItem => tempItem.color === item.color)

    if (foundItem) {
      foundItem.end = item.date
      resRanges.push({...foundItem})
      tempRanges.splice(tempRanges.findIndex(item => item.color === foundItem.color), 1)
    } else {
      const newItem: Range = {start: item.date, color: item.color, end: ''}
      tempRanges.push(newItem)
    }
  })

  return resRanges.sort((a, b) => new Date(a.start) > new Date(b.start) ? 1 : -1);
};
